/*
Story Engine API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.1.12
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package storyteller

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the StoryDetailResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &StoryDetailResponse{}

// StoryDetailResponse Detailed story response for admin and authoring endpoints.  Includes the declarative state persistence fields: character registry, location registry, object registry, flag definitions, initial state, and the original generation contract. ``generation_contract`` is ``None`` for stories created before migration 017.
type StoryDetailResponse struct {
	StoryId string `json:"story_id"`
	Title string `json:"title"`
	StartNodeId string `json:"start_node_id"`
	Genre NullableString `json:"genre,omitempty"`
	ContentWarnings []string `json:"content_warnings,omitempty"`
	MaxNodes *int32 `json:"max_nodes,omitempty"`
	Unbounded *bool `json:"unbounded,omitempty"`
	FlagSchema map[string]interface{} `json:"flag_schema,omitempty"`
	InitialWorldFlags map[string]interface{} `json:"initial_world_flags,omitempty"`
	Characters map[string]CharacterSpecResponse `json:"characters,omitempty"`
	Locations map[string]LocationSpecResponse `json:"locations,omitempty"`
	Objects map[string]ObjectSpecResponse `json:"objects,omitempty"`
	GenerationContract map[string]interface{} `json:"generation_contract,omitempty"`
}

type _StoryDetailResponse StoryDetailResponse

// NewStoryDetailResponse instantiates a new StoryDetailResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewStoryDetailResponse(storyId string, title string, startNodeId string) *StoryDetailResponse {
	this := StoryDetailResponse{}
	this.StoryId = storyId
	this.Title = title
	this.StartNodeId = startNodeId
	var maxNodes int32 = 10
	this.MaxNodes = &maxNodes
	var unbounded bool = false
	this.Unbounded = &unbounded
	return &this
}

// NewStoryDetailResponseWithDefaults instantiates a new StoryDetailResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewStoryDetailResponseWithDefaults() *StoryDetailResponse {
	this := StoryDetailResponse{}
	var maxNodes int32 = 10
	this.MaxNodes = &maxNodes
	var unbounded bool = false
	this.Unbounded = &unbounded
	return &this
}

// GetStoryId returns the StoryId field value
func (o *StoryDetailResponse) GetStoryId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.StoryId
}

// GetStoryIdOk returns a tuple with the StoryId field value
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetStoryIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.StoryId, true
}

// SetStoryId sets field value
func (o *StoryDetailResponse) SetStoryId(v string) {
	o.StoryId = v
}

// GetTitle returns the Title field value
func (o *StoryDetailResponse) GetTitle() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Title
}

// GetTitleOk returns a tuple with the Title field value
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Title, true
}

// SetTitle sets field value
func (o *StoryDetailResponse) SetTitle(v string) {
	o.Title = v
}

// GetStartNodeId returns the StartNodeId field value
func (o *StoryDetailResponse) GetStartNodeId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.StartNodeId
}

// GetStartNodeIdOk returns a tuple with the StartNodeId field value
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetStartNodeIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.StartNodeId, true
}

// SetStartNodeId sets field value
func (o *StoryDetailResponse) SetStartNodeId(v string) {
	o.StartNodeId = v
}

// GetGenre returns the Genre field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *StoryDetailResponse) GetGenre() string {
	if o == nil || IsNil(o.Genre.Get()) {
		var ret string
		return ret
	}
	return *o.Genre.Get()
}

// GetGenreOk returns a tuple with the Genre field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *StoryDetailResponse) GetGenreOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Genre.Get(), o.Genre.IsSet()
}

// HasGenre returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasGenre() bool {
	if o != nil && o.Genre.IsSet() {
		return true
	}

	return false
}

// SetGenre gets a reference to the given NullableString and assigns it to the Genre field.
func (o *StoryDetailResponse) SetGenre(v string) {
	o.Genre.Set(&v)
}
// SetGenreNil sets the value for Genre to be an explicit nil
func (o *StoryDetailResponse) SetGenreNil() {
	o.Genre.Set(nil)
}

// UnsetGenre ensures that no value is present for Genre, not even an explicit nil
func (o *StoryDetailResponse) UnsetGenre() {
	o.Genre.Unset()
}

// GetContentWarnings returns the ContentWarnings field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *StoryDetailResponse) GetContentWarnings() []string {
	if o == nil {
		var ret []string
		return ret
	}
	return o.ContentWarnings
}

// GetContentWarningsOk returns a tuple with the ContentWarnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *StoryDetailResponse) GetContentWarningsOk() ([]string, bool) {
	if o == nil || IsNil(o.ContentWarnings) {
		return nil, false
	}
	return o.ContentWarnings, true
}

// HasContentWarnings returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasContentWarnings() bool {
	if o != nil && !IsNil(o.ContentWarnings) {
		return true
	}

	return false
}

// SetContentWarnings gets a reference to the given []string and assigns it to the ContentWarnings field.
func (o *StoryDetailResponse) SetContentWarnings(v []string) {
	o.ContentWarnings = v
}

// GetMaxNodes returns the MaxNodes field value if set, zero value otherwise.
func (o *StoryDetailResponse) GetMaxNodes() int32 {
	if o == nil || IsNil(o.MaxNodes) {
		var ret int32
		return ret
	}
	return *o.MaxNodes
}

// GetMaxNodesOk returns a tuple with the MaxNodes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetMaxNodesOk() (*int32, bool) {
	if o == nil || IsNil(o.MaxNodes) {
		return nil, false
	}
	return o.MaxNodes, true
}

// HasMaxNodes returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasMaxNodes() bool {
	if o != nil && !IsNil(o.MaxNodes) {
		return true
	}

	return false
}

// SetMaxNodes gets a reference to the given int32 and assigns it to the MaxNodes field.
func (o *StoryDetailResponse) SetMaxNodes(v int32) {
	o.MaxNodes = &v
}

// GetUnbounded returns the Unbounded field value if set, zero value otherwise.
func (o *StoryDetailResponse) GetUnbounded() bool {
	if o == nil || IsNil(o.Unbounded) {
		var ret bool
		return ret
	}
	return *o.Unbounded
}

// GetUnboundedOk returns a tuple with the Unbounded field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetUnboundedOk() (*bool, bool) {
	if o == nil || IsNil(o.Unbounded) {
		return nil, false
	}
	return o.Unbounded, true
}

// HasUnbounded returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasUnbounded() bool {
	if o != nil && !IsNil(o.Unbounded) {
		return true
	}

	return false
}

// SetUnbounded gets a reference to the given bool and assigns it to the Unbounded field.
func (o *StoryDetailResponse) SetUnbounded(v bool) {
	o.Unbounded = &v
}

// GetFlagSchema returns the FlagSchema field value if set, zero value otherwise.
func (o *StoryDetailResponse) GetFlagSchema() map[string]interface{} {
	if o == nil || IsNil(o.FlagSchema) {
		var ret map[string]interface{}
		return ret
	}
	return o.FlagSchema
}

// GetFlagSchemaOk returns a tuple with the FlagSchema field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetFlagSchemaOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.FlagSchema) {
		return map[string]interface{}{}, false
	}
	return o.FlagSchema, true
}

// HasFlagSchema returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasFlagSchema() bool {
	if o != nil && !IsNil(o.FlagSchema) {
		return true
	}

	return false
}

// SetFlagSchema gets a reference to the given map[string]interface{} and assigns it to the FlagSchema field.
func (o *StoryDetailResponse) SetFlagSchema(v map[string]interface{}) {
	o.FlagSchema = v
}

// GetInitialWorldFlags returns the InitialWorldFlags field value if set, zero value otherwise.
func (o *StoryDetailResponse) GetInitialWorldFlags() map[string]interface{} {
	if o == nil || IsNil(o.InitialWorldFlags) {
		var ret map[string]interface{}
		return ret
	}
	return o.InitialWorldFlags
}

// GetInitialWorldFlagsOk returns a tuple with the InitialWorldFlags field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetInitialWorldFlagsOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.InitialWorldFlags) {
		return map[string]interface{}{}, false
	}
	return o.InitialWorldFlags, true
}

// HasInitialWorldFlags returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasInitialWorldFlags() bool {
	if o != nil && !IsNil(o.InitialWorldFlags) {
		return true
	}

	return false
}

// SetInitialWorldFlags gets a reference to the given map[string]interface{} and assigns it to the InitialWorldFlags field.
func (o *StoryDetailResponse) SetInitialWorldFlags(v map[string]interface{}) {
	o.InitialWorldFlags = v
}

// GetCharacters returns the Characters field value if set, zero value otherwise.
func (o *StoryDetailResponse) GetCharacters() map[string]CharacterSpecResponse {
	if o == nil || IsNil(o.Characters) {
		var ret map[string]CharacterSpecResponse
		return ret
	}
	return o.Characters
}

// GetCharactersOk returns a tuple with the Characters field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetCharactersOk() (map[string]CharacterSpecResponse, bool) {
	if o == nil || IsNil(o.Characters) {
		return map[string]CharacterSpecResponse{}, false
	}
	return o.Characters, true
}

// HasCharacters returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasCharacters() bool {
	if o != nil && !IsNil(o.Characters) {
		return true
	}

	return false
}

// SetCharacters gets a reference to the given map[string]CharacterSpecResponse and assigns it to the Characters field.
func (o *StoryDetailResponse) SetCharacters(v map[string]CharacterSpecResponse) {
	o.Characters = v
}

// GetLocations returns the Locations field value if set, zero value otherwise.
func (o *StoryDetailResponse) GetLocations() map[string]LocationSpecResponse {
	if o == nil || IsNil(o.Locations) {
		var ret map[string]LocationSpecResponse
		return ret
	}
	return o.Locations
}

// GetLocationsOk returns a tuple with the Locations field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetLocationsOk() (map[string]LocationSpecResponse, bool) {
	if o == nil || IsNil(o.Locations) {
		return map[string]LocationSpecResponse{}, false
	}
	return o.Locations, true
}

// HasLocations returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasLocations() bool {
	if o != nil && !IsNil(o.Locations) {
		return true
	}

	return false
}

// SetLocations gets a reference to the given map[string]LocationSpecResponse and assigns it to the Locations field.
func (o *StoryDetailResponse) SetLocations(v map[string]LocationSpecResponse) {
	o.Locations = v
}

// GetObjects returns the Objects field value if set, zero value otherwise.
func (o *StoryDetailResponse) GetObjects() map[string]ObjectSpecResponse {
	if o == nil || IsNil(o.Objects) {
		var ret map[string]ObjectSpecResponse
		return ret
	}
	return o.Objects
}

// GetObjectsOk returns a tuple with the Objects field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StoryDetailResponse) GetObjectsOk() (map[string]ObjectSpecResponse, bool) {
	if o == nil || IsNil(o.Objects) {
		return map[string]ObjectSpecResponse{}, false
	}
	return o.Objects, true
}

// HasObjects returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasObjects() bool {
	if o != nil && !IsNil(o.Objects) {
		return true
	}

	return false
}

// SetObjects gets a reference to the given map[string]ObjectSpecResponse and assigns it to the Objects field.
func (o *StoryDetailResponse) SetObjects(v map[string]ObjectSpecResponse) {
	o.Objects = v
}

// GetGenerationContract returns the GenerationContract field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *StoryDetailResponse) GetGenerationContract() map[string]interface{} {
	if o == nil {
		var ret map[string]interface{}
		return ret
	}
	return o.GenerationContract
}

// GetGenerationContractOk returns a tuple with the GenerationContract field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *StoryDetailResponse) GetGenerationContractOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.GenerationContract) {
		return map[string]interface{}{}, false
	}
	return o.GenerationContract, true
}

// HasGenerationContract returns a boolean if a field has been set.
func (o *StoryDetailResponse) HasGenerationContract() bool {
	if o != nil && !IsNil(o.GenerationContract) {
		return true
	}

	return false
}

// SetGenerationContract gets a reference to the given map[string]interface{} and assigns it to the GenerationContract field.
func (o *StoryDetailResponse) SetGenerationContract(v map[string]interface{}) {
	o.GenerationContract = v
}

func (o StoryDetailResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o StoryDetailResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["story_id"] = o.StoryId
	toSerialize["title"] = o.Title
	toSerialize["start_node_id"] = o.StartNodeId
	if o.Genre.IsSet() {
		toSerialize["genre"] = o.Genre.Get()
	}
	if o.ContentWarnings != nil {
		toSerialize["content_warnings"] = o.ContentWarnings
	}
	if !IsNil(o.MaxNodes) {
		toSerialize["max_nodes"] = o.MaxNodes
	}
	if !IsNil(o.Unbounded) {
		toSerialize["unbounded"] = o.Unbounded
	}
	if !IsNil(o.FlagSchema) {
		toSerialize["flag_schema"] = o.FlagSchema
	}
	if !IsNil(o.InitialWorldFlags) {
		toSerialize["initial_world_flags"] = o.InitialWorldFlags
	}
	if !IsNil(o.Characters) {
		toSerialize["characters"] = o.Characters
	}
	if !IsNil(o.Locations) {
		toSerialize["locations"] = o.Locations
	}
	if !IsNil(o.Objects) {
		toSerialize["objects"] = o.Objects
	}
	if o.GenerationContract != nil {
		toSerialize["generation_contract"] = o.GenerationContract
	}
	return toSerialize, nil
}

func (o *StoryDetailResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"story_id",
		"title",
		"start_node_id",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varStoryDetailResponse := _StoryDetailResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varStoryDetailResponse)

	if err != nil {
		return err
	}

	*o = StoryDetailResponse(varStoryDetailResponse)

	return err
}

type NullableStoryDetailResponse struct {
	value *StoryDetailResponse
	isSet bool
}

func (v NullableStoryDetailResponse) Get() *StoryDetailResponse {
	return v.value
}

func (v *NullableStoryDetailResponse) Set(val *StoryDetailResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableStoryDetailResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableStoryDetailResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStoryDetailResponse(val *StoryDetailResponse) *NullableStoryDetailResponse {
	return &NullableStoryDetailResponse{value: val, isSet: true}
}

func (v NullableStoryDetailResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStoryDetailResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


